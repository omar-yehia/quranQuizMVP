...
def get_ayas_ranges_for_suras(cursor, page):
	query = """
		SELECT MIN(id) AS start_id, MAX(id) AS end_id, sura_id
	...
	return [
		{'ayas_ids_range': [row[0], row[1]], 'sura_id': row[2]}
		for row in results
	]
...
def create_js_file(cursor, page):
	...
	with open(f"{page}.js", "w", encoding='utf-8') as file:
		...
		# Fetch ranges of ayas for each sura on the page
		ayas_ranges = get_ayas_ranges_for_suras(cursor, page)

		# Write the ayas ranges data
		file.write("\nconst ayasRanges = ")
		file.write(json.dumps(ayas_ranges, ensure_ascii=False))
		file.write(";\n")
